...
def test_p(
    ...
    opt_A = torch.optim.LBFGS([A_perturb], max_iter=20)  #
    ...
    A_total = A + A_mask * A_perturb
    X_opt_temp = torch.linalg.solve(A_total.T @ A_total, A_total.T @ y.T)
    final_loss = torch.mean((y - X_opt_temp.T @ A_total.T) ** 2).item()

    return final_loss, X_opt_temp - x.T
